...
## ao 残課題の常駐化（2026-09-14、owner 指示「それぞれ bot profile に任せる」）
...
本命は **Holo3.1-35B-A3B Q4 GGUF**。active 3B なので dense 27B (xavier 2.6 tok/s) より
decode が大幅に速い見込み — **要 node 実測**。接続案: alias `murakumo/cua` →
concrete `holo3.1-35b-a3b-gguf`。32GB tier（xavier / gad head）が第一候補。
24GB 6600h は Vulkan 12.2GB に対し I-Mini ~13GB で載らない可能性が高い。
⚠ 事前 gate: ①node 実測（decode tok/s + image step latency）②mmproj が llama.cpp
build で screenshot 入力できること ③itonami.cloud 実画面での click 坐標 mini probe。
...
## watch 報告のラベル区別（murakumo・codinator 合意、2026-09-18）

- ling-3.0-tiny は usagi ノード（aiueos-node-260910-usagi, :8096）が Mac/gad 双方から
  到達不能・tailscale 名も未解決の間、「unverified」ではなく
  **「node down で検証不能（verify 不能の原因は registry 側でなくノード側）」と
  表記する**。verified_at=null は正しい状態。tailscale 解決不可は
  owner / dns-nameserver-ops 管轄にエスカレーション済み。
- 台帳の `fresh` 欄は gateway の status-evidence.verdict の verbatim 写し
  （現状 ling は "unverified"）なので、probe 失敗 vs node Down の区別は**報告文**で
  付ける（台帳は script が書く — watch は観測のみ、gateway を書換えない）。
  台帳フィールドレベルの区別が要るなら gateway 側 status-evidence に node 到達性が
  入るのを待つ。
- hokusai 系は verified-faces=image（t2v/video 面は UNMEASURED のまま）が意図された
  状態。image だけ fresh に見えるのは正常であり drift として報告しない
  （cloud-murakumo-api#270、2026-09-18、hokusai-weekly で verdict=fresh 確認予定）。
